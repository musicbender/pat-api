...
import {
  ...
  deleteMutation,
  updateMutation,
} from '@mocks/queries/body-temperature';
import * as moment from 'moment';
import { gqlPlugin } from '@integration/lib/plugins';
const inputs = require('@mocks/inputs/body-temperature.json');
...
describe('Health - Body Temperature', () => {
  const CONFIG_ID = 'body-temperature';
  ...
  describe('query', () => {
    it('query for item 1', async () => {
      ...
      const { response: data } = res.body.data.bodyTemperature;
  ...
  describe('update mutation', () => {
    it('can change value for item 1', async () => {
      ...
      const { response: data } = res.body.data.updateBodyTemperature;

      expect(data.id).toEqual(itemIDs[0]);
      expect(data.value).toEqual(101.2);
      expect(moment(data.sampledOn).isValid()).toBeTruthy();
      expect(moment(data.createdOn).isValid()).toBeTruthy();
      expect(moment(data.updatedOn).isValid()).toBeTruthy();
      expect(data.configID).toEqual(CONFIG_ID);
    ...
    it('can change value for item 2', async () => {
      ...
      expect(data.id).toEqual(itemIDs[1]);
      expect(data.value).toEqual(98.0);
      expect(moment(data.sampledOn).isValid()).toBeTruthy();
      expect(moment(data.createdOn).isValid()).toBeTruthy();
      expect(moment(data.updatedOn).isValid()).toBeTruthy();
      expect(data.configID).toEqual(CONFIG_ID);
  ...
  describe('delete mutation', () => {
    it('can remove item 1', async () => {
      ...
      const { response: data } = res.body.data.deleteBodyTemperature;
